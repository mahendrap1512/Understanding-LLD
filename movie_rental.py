...
class MovieRentalSystem:
    ...
    def _find_movie_in_store(self, store_id: int, movie_id: int):
        return next(
            (
                ...
                if movie_copy.store_id == store_id and movie_copy.movie_id == movie_id
            ),
            None,
        )
    ...
    def search(self, movie_id: str):
        ...
        return sorted(result, key=lambda movie_copy: movie_copy.price)[
    ...
    def return_movie(self, store_id, movie_id):
        ...
        print(
            f"movie {movie_id} returned to store {store_id} successfully with rent is {rental.fare}"
        )
    ...
    def report(self, report_type: str):
        report = Report()
        return report.get_report_mapping(report_type)(
            report, self._movie_copies, self._rentals.values(), 5
        )
...
class Report:
    ...
    def _add_movies_from_rentals(
        self, movie_copies: Dict[int, MovieCopy], rentals: List[Rental]
    ):
        movies = []
    ...
    def cheapest_movie(
        self,
        movie_copies: Dict[int, MovieCopy],
        rentals: List[Rental],
        _limit: int,
    ):
        ...
        print(
            "cheapest movies",
            self._add_movies_from_rentals(movie_copies, cheapest_rentals),
        )

    def most_rented_movies(
        self,
        movie_copies: Dict[int, MovieCopy],
        rentals: List[Rental],
        _limit: int,
        ...
        most_rented_movies = sorted(
            movies_count, key=lambda x: movies_count[x], reverse=True
        )[:_limit]
